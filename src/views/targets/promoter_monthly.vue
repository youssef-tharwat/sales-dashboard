<template>
  <div id="promoter-target-monthly">
    <div class="row">
      <div class="col-lg-12">
        <div class="row">
          <div class="col-3">
        ...
        <div class="row">
          <div class="col-lg-12">
            <VueBootstrap4Table
              ...
              @refresh-data="getPromoterDataList"
            >
...
export default {
  name: "TargetPromoterMonthlyData",
  components: {
    BModal: BModal,
  ...
  data() {
    return {
      ...
      content:
        "Your EXCEL data file should contain all the columns specific to the operation you are performing. The sample EXCEL file has most of the columns required for the operation." +
        "Do not edit the column labels in the sample file and make sure the file extension is .xlsx file.",
      config: {
        checkbox_rows: false,
  ...
  methods: {
    ...
    setTargetTypes() {
      this.$store.dispatch(TARGET_SALE_TYPE_REQUEST, {}).then(res => {
        if (res.resultCode === 0) {
          ...
          // alert(JSON.stringify(this.targetTypes[0].id));
          this.target_type = res.content[0].id;
    ...
    getPromoterDataList(postd) {
      ...
      this.$store
        .dispatch(GET_PROMOTER_TARGET_DATA_LIST_REQUEST, postd)
        .then(res => {
          if (res.resultCode === 0) {
            this.formatDataArray(res.content.targetByPromoterList);
          }
    ...
    setDefaultData() {
      ...
      if (!this.target_type) {
        // alert(JSON.stringify(this.targetTypes[0]));
    ...
    setPostData(pdata) {
      ...
      pdata.forEach(function(row) {
        ...
        let type = types.filter(function(elem) {
          if (elem.text.toLowerCase() === row[2].toLowerCase()) return elem;
        });
    ...
    getPromoterList() {
      this.$store
        .dispatch(GET_STAFF_LIST_REQUEST, { type: "promoters" })
        .then(res => {
  ...
  mounted() {
    ...
    // alert(this.target_type);
    if (this.target_year && this.target_type) {
      this.getPromoterDataList(tdata);
    }

    // this.setExcelSampleData(this.allPromoters);
    // this.getPromoterList();
  }
};
</script>
