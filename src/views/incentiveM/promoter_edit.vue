<template>
  <div id="incentive-promoter-edit">
    <breadcrumb
      title="Dashboard"
      subtitle="Promoter Incentive"
      sub-sub-title="Edit Promoter Incentive"
    ></breadcrumb>
              ...
              <form>
                <div class="row form-group">
                  ...
                  <div class="col-6 mb-4">
                    ...
                    <span class="invalid-field mt-2">{{ errors.first("Start Month") }}</span>
                    <span v-if="incentiveErrors.startMonth" class="invalid-field mt-2">{{
                      incentiveErrors.startMonth
                    }}</span>
                  </div>
                  <div class="col-6 mb-4">
                    ...
                    <span class="invalid-field mt-2">{{ errors.first("End Month") }}</span>
                    <span v-if="incentiveErrors.endMonth" class="invalid-field mt-2">{{
                      incentiveErrors.endMonth
                    }}</span>
                  </div>
                </div>
                ...
                <div class="col-12 mb-4 d-flex align-items-center p-0">
                  <div class="d-flex flex-column mr-5 mt-4">
                    <div class="d-flex mb-3">
                      ...
                      <div v-if="!isCountryAdmin" class="ml-3">
                        <i
                          v-if="tiersCount < 5"
                      ...
                      <div class="d-flex flex-column">
                        ...
                        <div class="d-flex align-items-center">
                          <div
                            ...
                            class="mr-4"
                          >
                            {{ tierIndex >= 0 ? "Tier " + tierCounter(tierIndex) : "" }}
                            <div>
                              <input
                              ...
                              />
                              <span class="invalid-field mt-2">{{
                                errors.first("Target Tier " + tierCounter(tierIndex))
                              }}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="d-flex align-items-center">
                          ...
                          >
                            ...
                            <div>
                              ...
                              />
                              <span class="invalid-field mt-2">{{
                                errors.first("Reward Tier " + tierCounter(index2))
                              }}</span>
                            </div>
                          </div>
                        </div>
                ...
                <div class="col-12 mb-4 d-flex align-items-center p-0">
                  <div class="d-flex flex-fill flex-column mt-4">
                    <div class="d-flex mb-3">
                      <h5 class="black-text">Category</h5>
                      <div v-if="!isCountryAdmin" class="ml-3">
                        <i
                          v-if="tiersCount2 < 5"
                          class="fa fa-plus-circle"
                              ...
                              >
                                ...
                                <td class="w-75">
                                  <div class="float-left">
                                    <span
                                      v-for="(product, idx) in tierItem.productFamily"
                                      :key="'C' + idx"
                                      scope="col"
                                      class="mr-1"
                                    >
                                    ...
                                    >
                                      The Product Family field is required
                                    </span>
                                  </div>
                                  <div v-if="!isCountryAdmin" class="float-right">
                                    <button
                                      type="button"
                                ...
                                <td>
                                  ...
                                  />
                                  <span class="invalid-field mt-2">{{
                                    errors.first("Tier " + tierCounter(tierIndex))
                                  }}</span>
                                </td>
                              </tr>
                            </tbody>
                ...
                <div class="row mt-5">
                  <div class="col-12">
                    ...
                    <div v-else>
                      <router-link
                        to="/dashboard/incentive/promoter"
                        class="btn btn-primary pull-right ml-3"
                      >
                        Back
                      </router-link>
    ...
    <BModal ref="selection-modal" size="lg" hide-footer title="Product Family">
      ...
      <div class="d-flex align-items-center justify-content-end">
        <button type="btn" class="btn btn-primary btn-sm" @click="modalToggle(null)">
          Close
        </button>
      </div>
    </BModal>
...
export default {
  ...
  methods: {
    async handleSubmit() {
      ...
      this.$validator.validateAll().then(result => {
        if (result && Object.keys(this.incentiveErrors).length === 0) {
          this.dataAdjustment();
          this.$store.dispatch(INCENTIVE_PROMOTER_UPDATE, this.incentive).then(res => {
            this.submitted = true;
            if (res.resultCode === 0) {
              this.$router.push("/dashboard/incentive/promoter");
            } else {
              this.incentive.startMonth = this.incentive.startMonthString;
              this.incentive.endMonth = this.incentive.endMonthString;
            }
          });
        }
      });
    ...
    dataAdjustment() {
      ...
      this.incentive.objective2List = [...this.unformatTableRowData(adjustedObj2Array)];
    ...
    checkDates() {
      if (this.incentive.startMonth && this.incentive.endMonth) {
        const startDate = moment(this.incentive.startMonth).format("YYYY-MM-DD");
        ...
        if (!moment(startDate).isSame(endDate) && !moment(startDate).isBefore(endDate)) {
          this.incentiveErrors.startMonth = "Start month must be before end month";
        } else {
          delete this.incentiveErrors.startMonth;
        }

        if (!moment(endDate).isSame(startDate) && !moment(endDate).isAfter(startDate)) {
          this.incentiveErrors.endMonth = "End month must be after or equal to start month";
        } else {
    ...
    async getIncentive() {
      const incentiveId = this.$route.params.id;
      const res = await this.$store.dispatch(INCENTIVE_PROMOTER_GET_BY_ID, incentiveId);
      if (res.resultCode === 0) {
        const data = JSON.parse(JSON.stringify(res.content));
        this.incentive = data;
        this.incentive.startMonth = data.startMonthString;
        this.incentive.endMonth = data.endMonthString;
        this.adjustedArray();
      }
    },
    adjustedArray() {
      const tArray = JSON.parse(JSON.stringify(this.incentive.tierList));
      const adjustedArray = tArray.sort((a, b) => a.tierLevel - b.tierLevel).map(item => item);

      this.tiersArray = adjustedArray;
      this.tiersCount = adjustedArray.length;

      const tArray2 = JSON.parse(JSON.stringify(this.incentive.objective2List));
      if (tArray2) {
        this.tiersCount2 = tArray2.length;
        const adjustArray2 = tArray2.map(obj => {
          obj.productFamily = [...this.formatData(obj.productFamily)];
          return obj;
        });
        this.tiersArray2 = [...adjustArray2];
      }
    },
    validateObj2() {
    ...
    sortingList() {
      let available = this.availProductFamilyList;
      let selected = this.selectedProductFamily.map(product => {
        return product.name;
      });
      let final = available.sort((a, b) => {
        return selected.indexOf(b) - selected.indexOf(a);
      });
      this.availProductFamilyList = final;
    },
    ...
    async getProductFamilyList() {
      ...
      const res = await this.$store.dispatch(INCENTIVE_PROMOTER_PRODUCT_FAMILY_REQUEST);
      this.productFamilyList = await res.content.filter(item => item);
      this.availProductFamilyList = await JSON.parse(JSON.stringify(this.productFamilyList));
