<template>
  <div id="state-manager-list">
    <breadcrumb title="Dashboard" subtitle="Staff Management" sub-sub-title="State Manager" />
              ...
              <div class="row">
                <div class="col-lg-12 add-employee-button d-flex align-items-center">
                  ...
                  <button v-if="canAdd" class="btn btn-sm btn-primary" @click="toggleModal(null)">
                    ...
                    <span v-text="$ml.get('add_state_manager_title')"></span>
                  </button>
                  ...
                  >
                    ...
                    <template slot="empty-results">
                      <div
                        v-if="!isDataLoading"
                        class="bold-text"
                        v-text="$ml.get('no_state_managers_found')"
                      ></div>
                    ...
                    <template slot="Action" slot-scope="props">
                      <div class="inner-buttons btn-group float-right" role="group">
                        ...
                        <button class="btn btn-info btn-xs" @click="loadStates(props.row)">
                          <i class="fa fa-users"></i>
                          STATES ({{ props.row.assignedState.length }})
                        </button>
    ...
    <BModal ref="state-manager-modal" size="lg" hide-footer :title="setFormTitle">
      <stateManagerForm
        :can-edit="canEdit"
        :can-view="canView"
    ...
    <BModal ref="states-modal" size="lg" hide-footer title="Assigned States">
      <ManagerStates
        :close-modal="toggleStateModal"
        :manager-selected="manager"
        :load-data="getManagerList"
        :can-edit="canEdit"
        :can-view="canView"
...
import stateManagerForm from "../../components/staff/state_manager_form";
...
import ManagerStates from "../../components/staff/manager_states";
...
export default {
  ...
  components: {
    ...
    stateManagerForm: stateManagerForm,
    BModal: BModal,
  ...
  data() {
    return {
      ...
      sampleFile: "stateManagerUpload.xlsx",
      content:
      ...
      canEdit: false,
      canView: false,
      setFormTitle: "Add State Manager",
      currectAction: false,
  ...
  methods: {
    showModal() {
      this.manager = {};
      this.$refs["state-manager-modal"].show();
    },
    hideModal() {
      this.manager = {};
      this.$refs["state-manager-modal"].hide();
    },
    toggleModal(row, view) {
      if (row) {
        if (view) {
          this.setFormTitle = "View State Manager";
          this.currectAction = "view";
        } else {
          this.setFormTitle = "Edit State Manager";
          this.currectAction = "edit";
        }
        this.manager = row;
      } else {
        this.setFormTitle = "Add State Manager";
        this.currectAction = "edit";
        this.manager = {};
      }

      this.$refs["state-manager-modal"].toggle("#toggle-btn");
    },
    toggleStateModal() {
      this.$refs["states-modal"].toggle("#toggle-btn3");
    },
    ...
    getStateNamesByCode(codes) {
      let lst = [];
      let states = this.stateList;
      codes.forEach(item => {
        const matched = states.filter(function(elem) {
          if (elem.code == item) return elem;
        });

        if (matched[0]) {
          lst.push(matched[0].text);
        }
      });
      return lst.join();
    },
    ...
    doExport() {
      ...
      const fileName = "stateManager";
      export_json_to_excel({
        header: tHeader,
        data,
        filename: fileName,
        sheetName: "stateManagers",
        autoWidth: true,
        bookType: "xlsx"
      });
    },
    formatJson(filterVal, jsonData) {
      return jsonData.map(v =>
        filterVal.map(j => {
          ...
          } else if (j === "assignedState") {
            return this.getStateNamesByCode(v[j]);
          } else {
            return v[j];
          }
        })
    ...
    setPostData(pdata) {
      ...
      if (this.postManagerData.length > 0) {
        this.submitStateManagerData();
      }
    },
    submitStateManagerData() {
      let pdata = {};
      pdata.type = "stateManagers";
      ...
      this.$store.dispatch(POST_STAFF_REQUEST, pdata).then(res => {
        if (res.resultCode === 0) {
          ...
          this.$store.dispatch(NOTIFICATION_SUCCESS, {
            message: "State Manager Successfully Added",
            timeOut: true
          });
        }
      });
