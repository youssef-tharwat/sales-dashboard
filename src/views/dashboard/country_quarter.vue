<template>
  <div v-if="hasLoaded && !isLoading" class="row w-100">
    ...
    <div class="col-lg-12 p-0">
      ...
      <!-- <div class="col-lg-6 float-left border d-flex align-items-center justify-content-center">
        <bar-chart
          :data.sync="dashboardData.lowestAchievement.data"
          :title="dashboardData.lowestAchievement.name"
          color="#b07aa1"
          :cap.sync="isCap"
          :number-of-charts-per-row="2"
          :percent="true"
        ></bar-chart>
      </div> -->
    </div>
  ...
  methods: {
    async getDashboardData() {
      if (!this.filters.quarterSelected) {
        this.filters.quarterSelected = await { name: "Q4", value: "Q4" };
      }
      this.isLoading = true;
      await this.$store.dispatch(CDASHBOARD_DATA_GET, {
        grouping: "quarterly",
    ...
    async getProductPerformanceDropDowns() {
      const res = await this.$store.dispatch(CDASHBOARD_PRODUCT_FILTERS_GET);
    ...
    async getComponentPerformanceDropDowns() {
      const res = await this.$store.dispatch(CDASHBOARD_COMPONENT_FILTERS_GET);
