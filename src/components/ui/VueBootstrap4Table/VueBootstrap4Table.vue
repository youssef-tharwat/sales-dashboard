  ...
  <div :class="{card:card_mode}">
    ...
    <div :class="{'card-body':card_mode}">
      ...
      <div class="row vbt-header-row no-gutters">
        ...
        <div class="col-md-8">
          <select
            ...
            v-model="per_page"
            @change="perPageHandler(per_page)"
          >
            <option
          ...
          <a
            class="btn btn-primary dropdown-toggle pageLimitBtn"
            href="#"
            role="button"
            id="dropdownMenuLink"
        ...
        <div class="col-md-4">
          <div class="row no-gutters">
            ...
            <div class="col-md-12 input-group vbt-global-search" v-if="global_search.visibility">
              <div class="input-group mb-3">
                ...
                />
                <div class="input-group-append" v-if="global_search.showClearButton">
                  <span
                ...
                <div class="input-group-append" v-if="show_reset_button">
                  <span
                    @click="resetQuery"
                ...
                <div class="input-group-append" v-if="show_refresh_button">
                  <span
                    @click="clearGlobalSearch(); $emit('refresh-data');"
                    class="input-group-text btn-success btn-reload"
                    id="basic-addon4"
                  >Refresh</span>
                </div>
              </div>
            ...
            <!-- <div class="col-md-6">
                                                <div class="btn-group" role="group" aria-label="Table Actions buttons">
                                                    <button type="button" class="btn btn-secondary btn-sm" @click="$emit('refresh-data')">
                                                        <slot name="refresh-button-text">
                                                            Refresh
                                                        </slot>
                                                    </button>
                                                    <button type="button" class="btn btn-secondary btn-sm" @click="resetQuery">
      ...
      <div :class="tableWrapperClasses" class="vbt-table-wrapper">
        <table class="table fixed" :class="tableClasses">
          <thead>
            <tr>
                    ...
                    <div class="text-left justify-content-center">
                      <template v-if="pagination_info">
                        ...
                        >
                          ...
                          >{{ (page * per_page ) <= filteredRowsLength ? (page * per_page) : filteredRowsLength }} of {{filteredRowsLength}} entries</template>
                          <template v-else>No results found</template>
                          <template>
                            <span class="ml-2">({{originalRowsLength}} total records)</span>
                          </template>
                        </slot>
                      </template>
...
import VueHighlight from "@/components/ui/vue-highlight";
      ...
      let res = filter(this.original_rows, row => {
        ...
        this.query.filters.some((filter, key) => {
          if (filter.type === "simple") {
            ...
            if (
              this.multiSelectFilter(get(row, filter.name), filter.selected_options, filter.config)
            ) {
              flag = true;
            } else {
              flag = false;
