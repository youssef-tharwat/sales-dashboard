...
export default {
  ...
  props: {
    ...
    className: {
      type: String,
      default:
        "table table-striped employee-contact-table table-responsive-fix-big default-ordering"
    },
    /**
  ...
  created() {
    ...
    let startCol = 0;
    let icol = 0;
    vm.tableId = vm.id || `lnvtable${myUniqueId++}`;
  ...
  mounted() {
    ...
    $el.on("click", "[data-action]", e => {
      ...
      while (!action) {
        ...
        if (
          that.hasClass("vdtnet-container") ||
